...
def parse_byte_message(msg, meth=get_key_value):
    # when receiving a message in byte form (i.e. from serial or network,
    # parse it according to the MSG_DELIMITER to get individual key-value messages
    # * CASES (if there ARE K-V msgs):
    #    - no delimiters: there is only one message
    #    - at least one delimiter: there are at least two messages
    # * CASES (if there are NO K-V msgs):
    #    - no KV-DELIMITER: no messages
...
def int_to_bytes(int_msg):
    return int_msg.to_bytes(2, "big")
