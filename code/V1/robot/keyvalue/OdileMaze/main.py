...
def add_esp_channels():
    ...
    control.on_new_config_rcv(VR_ip, ESP_VALUE_TYPE_KEYS.ANGLE_X.value, DofName.HEAD_UD.value.key, True)
    control.on_new_config_rcv(VR_ip, ESP_VALUE_TYPE_KEYS.ANGLE_Y.value, DofName.HEAD_LR.value.key, True)
...
def main_body():
    ...
    if CAMERA_ENABLED:
        if not PICAM:
            ...
            import test_picam
            camera_process = multiprocessing.Process(target=test_picam.capture_and_send, args=[])
        # start as daemon
